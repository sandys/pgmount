...
async fn test_format_json() {
    ...
    let json = openeral_core::format::json::format_row(&row).unwrap();
...
async fn test_format_json_rows() {
    ...
    let json = openeral_core::format::json::format_rows(&rows).unwrap();
    let parsed: serde_json::Value = serde_json::from_str(&json).unwrap();
...
async fn test_format_csv() {
    ...
    let csv = openeral_core::format::csv::format_row(&row).unwrap();
...
async fn test_format_yaml() {
    ...
    let yaml = openeral_core::format::yaml::format_row(&row).unwrap();
...
async fn test_pk_display_parsing() {
    use openeral_core::fs::nodes::column::parse_pk_display;
...
async fn test_pk_display_percent_encoding_roundtrip() {
    use openeral_core::db::queries::rows::encode_pk_value;
    use openeral_core::fs::nodes::column::parse_pk_display;
...
async fn test_migrations_create_openeral_schema() {
    ...
    // Verify _openeral schema exists
    let client = pool.get().await.unwrap();
    let rows = client
        .query(
            "SELECT schema_name FROM information_schema.schemata WHERE schema_name = '_openeral'",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT table_name FROM information_schema.tables \
             WHERE table_schema = '_openeral' AND table_name = 'mount_log'",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT table_name FROM information_schema.tables \
             WHERE table_schema = '_openeral' AND table_name = 'cache_hints'",
            &[],
        )
        .await
        .unwrap();
...
async fn test_log_mount_session() {
    ...
    let client = pool.get().await.unwrap();
    let rows = client
        .query("SELECT mount_point, schemas_filter, page_size, openeral_version FROM _openeral.mount_log", &[])
        .await
        .unwrap();
    assert_eq!(rows.len(), 1);
