...
impl MetadataCache {
    ...
    pub fn get_schemas(&self) -> Option<Vec<SchemaInfo>> {
        self.schemas.get("").and_then(|e| {
            if e.is_valid(self.ttl) { Some(e.data.clone()) } else { None }
        })
    }

    pub fn set_schemas(&self, schemas: Vec<SchemaInfo>) {
        self.schemas.insert("".to_string(), CacheEntry { data: schemas, inserted_at: Instant::now() });
    }
    ...
    pub fn set_tables(&self, schema: &str, tables: Vec<TableInfo>) {
        self.tables.insert(schema.to_string(), CacheEntry { data: tables, inserted_at: Instant::now() });
    }
    ...
    pub fn set_columns(&self, schema: &str, table: &str, columns: Vec<ColumnInfo>) {
        let key = format!("{}.{}", schema, table);
        self.columns.insert(key, CacheEntry { data: columns, inserted_at: Instant::now() });
    }
    ...
    pub fn set_primary_key(&self, schema: &str, table: &str, pk: PrimaryKeyInfo) {
        let key = format!("{}.{}", schema, table);
        self.primary_keys.insert(key, CacheEntry { data: pk, inserted_at: Instant::now() });
    }

    pub fn get_row_count(&self, schema: &str, table: &str) -> Option<i64> {
        let key = format!("{}.{}", schema, table);
        self.row_counts.get(&key).and_then(|e| {
            if e.is_valid(self.ttl) { Some(e.data) } else { None }
        })
    }

    pub fn set_row_count(&self, schema: &str, table: &str, count: i64) {
        let key = format!("{}.{}", schema, table);
        self.row_counts.insert(key, CacheEntry { data: count, inserted_at: Instant::now() });
    }
