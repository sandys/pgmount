...
/// 2. An environment variable named by `env_var` (e.g. `"PGMOUNT_DATABASE_URL"`).
/// 3. The first connection entry in `~/.pgmount/config.yml`.
///
...
pub fn resolve_connection_string(
    ...
    // 3. Config file at ~/.pgmount/config.yml
    if let Some(home) = home_dir() {
        let config_path = home.join(".pgmount").join("config.yml");
    ...
    Err(FsError::InvalidArgument(
        "No connection string provided. Pass one via CLI, set the environment variable, \
         or configure ~/.pgmount/config.yml"
            .to_string(),
    ))
}
